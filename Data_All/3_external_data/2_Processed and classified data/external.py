...
def build_double_attention_model(input_dim, num_classes=9):
...
def enhanced_cross_validation(features, labels, n_splits=10):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(features)):
        ...
        # Using the dynamically determined num_classes
        model = build_double_attention_model(features.shape[1], num_classes=num_classes)
        ...
        y_val_onehot_full = tf.one_hot(y_val, depth=num_classes).numpy()
        ...
        cm = confusion_matrix(y_val, y_pred_class, labels=np.arange(num_classes))
        class_acc = cm.diagonal() / cm.sum(axis=1)
        
        y_val_onehot = tf.one_hot(y_val, depth=num_classes).numpy() 
        class_roc_auc = []
...
if __name__ == "__main__":
    ...
    sequences, labels = load_and_oversample_data("E_9.csv")
    ...
    num_classes = len([k for k in final_metrics.keys() if k.startswith('Class_') and k.endswith('_ACC')])
    for i in range(num_classes):
